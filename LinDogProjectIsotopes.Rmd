...
# Let's assign Mutton dark purple and Lampchop green-blue when comparing the two of them alone.
dogs <- c("SB Village Dog" = "#1FA187FF", "Mutton" = "#440154FF")
dogshape <- c("SB Village Dog" = 15, "Mutton" = 17)
...
SBVillageDogBone <- subset(Converted, tissue == "Bone" & individual == "SB Village Dog")
...
df2$location2 <- factor(df2$location, levels = c("Mutton", "SB Village Dog", "Late-Holocene Deer", "Bridge River", "Broken Group Islands", "Cathlapotle", "Dionisio", "Gwaii Hannas", "Keatley Creek", "Namu", "Port Hardy", "Sliammon", "Sumas"))
```
...
When we plot the bone and hair of the two dogs, we see large differences between the two dogs, with bone and corrected hair values grouping together. SB Village Dog displays relatively restricted values across all samples for nitrogen and carbon, `r mean(subset(Converted, individual == "SB Village Dog")$N)` +- `r sd(subset(Converted, individual == "SB Village Dog")$N)` and `r mean(subset(Converted, individual == "SB Village Dog")$C_coll)` +- `r sd(subset(Converted, individual == "SB Village Dog")$C_coll)`, relatively.
...
```{r statsCompare}
#t.test((subset(Converted, individual == "Mutton")$C_coll), subset(Converted, individual == "SB Village Dog")$C_coll)

#t.test((subset(Converted, individual == "Mutton")$N), subset(Converted, individual == "SB Village Dog")$N)
summary(manova(cbind(C_coll, N) ~individual, data = Converted))
```
Though sample sizes are small, one-way MANOVA shows significant differences between SB Village Dog's and Mutton's stable isotope values. 
...
```
We can see that while SB Village Dog's bone values align with other PNW dogs, Mutton's bone value is suggestive of a terrestrial omnivore. 
...
For nitrogen, we see both dogs have small ranges in nitrogen values that hover near their bone values. SB Village Dog shows nitrogen hair values with a mean of  `r mean(subset(Converted, individual == "SB Village Dog" & tissue == "Hair")$N)` +- `r sd(subset(Converted, individual == "SB Village Dog" & tissue == "Hair")$N)`. With low SD, we can assume SB Village Dog ate at pretty much the same trophic level across the time period this hair represents. As with SB Village Dog's bone, we see higher values compared to Mutton. 
...
Compared to the nitrogen values, there appear to be little overlap between hair and bone carbon stable isotope values. For SB Village Dog, `r mean(subset(Converted, individual == "SB Village Dog" & tissue == "Hair")$C_coll)` +- `r sd(subset(Converted, individual == "SB Village Dog" & tissue == "Hair")$C_coll)` for hair compared to a bone carbon value of -14.53. Not hugely different, but not overlaps. With an SD of 0.47 for both carbon and nitrogen hair samples, SB Village Dog does not show great variation across time: they were consuming much the same types of foods across their live as recorded. 
...
For SB Village Dog, we see values suggestive of what we know about village dogs: a highly marine diet. Scavenging or being purposely fed similar foods to villagers, SB Village Dog did not show evident of seasonal shifts in food sources in their hair values. SB Village Dog's stable isotope values are similar to those of other archaeological-derived dogs from the broad region. 
...
Regarding Mutton's carbon values, we see the oldest hair sample display high carbon stable isotope value, and then a steep decline to the most recent hair sample, which is also in line with his bone value. High carbon might be reflective of marine values similar to SB Village Dog's, but without concomitant high nitrogen values that is unlikely unless it's something of a low trophic level such as edible seaweed. Instead, the high carbon values are likely a reflection of some C4 plant input. There are no wild edible C4 plants in the area to my knowledge, but nearby settlement forts would have had supplies such as sugarcane (in the form of refined sugar, or more likely molasses) and corn, both of which are C4 plants. 
...
legend("topleft", 
       legend = c("Mutton", "SB Village Dog"), 
       col = dogs, 
       pch = 16)
